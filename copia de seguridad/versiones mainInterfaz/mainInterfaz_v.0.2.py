...
def saveCA():
    try:
        ...
        site = siteName.get()
        techCA = techTextCA.get()
        otherTechs = otherTechTextCA.get()
        cells = numberSectorsTextCA.get()
        node = nodeTextCA.get()
        enodeB = enodeTextCA.get()
        if site == "":
...
def write_mail():
    ...
    mailText.insert(END,f"Hemos detectado las siguientes inconsistencias en el site {siteName.get()}:\n")
    ...
    for i in techText:
        if i.get() != "0":
            ...
            if optionList[cont] == "PUSCH":
                ...
                for j in listAux:
                    mailText.insert(END,f" {techText[cont].get()}{j}")
                    if int(j)<len(listAux):
                        mailText.insert(END,", ")
...
title_name_2 = Label(topPanel,text="Mail Generator  ",fg="black",font=("Dosis",22,"bold"),bg="SlateBlue",width=27) 
...
for option in optionList:
    ...
    option = Checkbutton(optionsPanel,text=option,font=("Arial",14,"bold"),onvalue=1,offvalue=0, variable=optionsVariables[cont],command=check_button)
    ...
    techText[cont].set("0")
    techBox[cont] = Entry(optionsPanel,font=("Arial",14,"bold"),bd=1,width=5,state=DISABLED,textvariable=techText[cont])
    ...
    sectorText[cont].set("0")
    sectorBox[cont] = Entry(optionsPanel,font=("Arial",14,"bold"),bd=1,width=11,state=DISABLED,textvariable=sectorText[cont])
...
for button in buttonsList:
    button = Button(buttonsPanel,text=button,font=("Arial",14,"bold"),fg="black",bg="Gray",bd=2,width=12)
...
mailText = Text(mailPanel,font=("Arial",10),fg="black",bd=1,width=63,height=29)
